...
merge m:m HHID2020 INDID2020 using "panel_indiv"
keep if _merge==3
drop _merge
...
* Merge caste
merge m:m HHID2020 using "raw\Panel-Caste_HH", keepusing(caste2020)
keep if _merge==3
drop _merge
*rename caste2020 caste
...
save"NEEMSIS2-HH~_ego.dta", replace
****************************************
* END
...
keep if _merge==3
drop _merge
order HHID_panel HHID2020 INDID_panel INDID2020
*destring INDID2020, replace
...
* Income
merge m:1 HHID2020 using "raw\NEEMSIS2-occup_HH", keepusing(incomeagri_HH incomenonagri_HH annualincome_HH shareincomeagri_HH shareincomenonagri_HH nbworker_HH nbnonworker_HH)
drop _merge
...
* Merge factor
merge 1:1 HHID_panel INDID_panel using "NEEMSIS2-HH~_ego.dta"
keep if _merge==3
drop _merge

foreach x in f1 f2 f3 f4 f5 {
rename `x'_2020 base_`x'
}
...
save"NEEMSIS2-HH~panel", replace
****************************************
* END
